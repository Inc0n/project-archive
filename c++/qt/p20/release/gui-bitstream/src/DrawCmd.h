...
// macro for serialize any data
#define SERIALIZE(buf_name, data) {             \
        memcpy(buf_name, &data, sizeof(data));  \
        buf_name += sizeof(data);               \
}

// must be used for end of serialize
// but not needed for deserialize
...
public:
    ...
    //
    DrawCmd(byte_t mode) : mode(mode) {}
    DrawCmd(QPointF p1, QPointF p2) : DrawCmd(0, p1, p2) {}
    DrawCmd(byte_t mode, QPointF p1, QPointF p2) {
        this->mode = mode;
        this->p1 = p1;
        this->p2 = p2;
    }
    ...
    DrawCmd(std::vector<char> data) : DrawCmd(&data[0]) {}
    //
